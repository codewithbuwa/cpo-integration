# CPO integration for HALOs — Stage 1 (unary CPO)

Drop-in extension that adds `CPOTrainer` to HALOs. Stage 2 (mixed
$\alpha \in (0, 1)$ regime with the DPO pairwise term) is intentionally
...
| Path | Type | Purpose |
|---|---|---|
| `train/cpo_trainer.py` | new file | `CPOTrainer(UnpairedPreferenceTrainer)` with per-cluster $z_k$ and EMA hook |
| `train/cpo_cluster.py` | new file | Sidecar-JSON cluster loader (pluggable; replace later) |
| `config/loss/cpo.yaml` | new file | Loss config; reproduces KTO when `num_clusters: 1` and `ema_tau: null` |
| `scripts/launch_llama_cpo.sh` | new file | Launch wrapper |

## What to drop into your HALOs checkout

Copy the four new files into the matching locations in your HALOs repo:
...
Then apply three small edits to existing files.
